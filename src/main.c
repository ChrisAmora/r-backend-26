...
static bool g_c_epoll_mode = false;
static int g_c_epoll_loops = 1;

static int g_c_epoll_idle_us = 0;
static uint32_t g_c_warmup_queries = 0;
...
static void init_response_lengths(void) {
...
static void c_instrument_init_from_env(void) {
...
static void log_anon_hugepages(const char *when) {
...
static uint8_t decide_frauds(const uint8_t *body, size_t body_len) {
    uint64_t t0 = c_now_ns();
    float v[DIMENSIONS];
...
static void init_detector_from_env(void) {
...
static void start_control_listener(const char *sock) {
...
static void epoll_serve(const char *sock_path) {
...
static void warmup_engine(uint32_t n_queries) {
